...
class FitCurves(CollectThenCompute):
    ...
    def __init__(self):
        super(FitCurves, self).__init__()
        self.fit_function = None
        self.fit_bounds = None
        self.results = []
        self.transform_function = None
        self.bounds = None # static bounds
        self.bounds_provider = None # function that provides bounds based on x and y values
    ...
    def compute(self):
        ...
        print("Scan shape : %s" % (str(scan_shape)))
        ...
        for i in range(0, num_events, readouts_per_row):
            param, cov = self.do_fitting(xvals[i:i + readouts_per_row], yvals[i:i + readouts_per_row])
...
def fit_quadratic_curve(data_results, show_plot=False, bounds=None, default_bounds=100, trial_quadratic=quadratic) :
    """
...
#    if xvals is None or yvals is None :
    ...
    y_vals = [v for v in data_results.values()]
    x_vals = [v for v in data_results.keys()]
